...
typedef std::tuple<vector<int>, vector<int>, double, char, double, int, bool> her_tuple;
...
Arguments setup_her_arguments(her_tuple tup)
{
...
class blas2_her_gtest : public ::TestWithParam<her_tuple>
{
protected:
    blas2_her_gtest() {}
    virtual ~blas2_her_gtest() {}
    virtual void SetUp() {}
    virtual void TearDown() {}
};

// her
TEST_P(blas2_her_gtest, her_gtest_float)
{
    ...
    {
        if(arg.N < 0 || arg.lda < arg.N || arg.incx == 0)
        {
...
TEST_P(blas2_her_gtest, her_gtest_double)
{
    // GetParam return a tuple. Tee setup routine unpack the tuple
    // and initializes arg(Arguments) which will be passed to testing routine
    // The Arguments data struture have physical meaning associated.
...
// her_batched
TEST_P(blas2_her_gtest, her_batched_gtest_float)
{
    // GetParam return a tuple. Tee setup routine unpack the tuple
    // and initializes arg(Arguments) which will be passed to testing routine
    // The Arguments data struture have physical meaning associated.
    ...
    if(status != HIPBLAS_STATUS_SUCCESS)
    {
        if(arg.N < 0 || arg.lda < arg.N || arg.incx == 0 || arg.batch_count < 0)
        {
            EXPECT_EQ(HIPBLAS_STATUS_INVALID_VALUE, status);
        }
        else
...
TEST_P(blas2_her_gtest, her_batched_gtest_double)
{
    // GetParam return a tuple. Tee setup routine unpack the tuple
    // and initializes arg(Arguments) which will be passed to testing routine
    // The Arguments data struture have physical meaning associated.
...
// her_strided_batched
TEST_P(blas2_her_gtest, her_strided_batched_gtest_float)
{
    // GetParam return a tuple. Tee setup routine unpack the tuple
    // and initializes arg(Arguments) which will be passed to testing routine
    // The Arguments data struture have physical meaning associated.
...
TEST_P(blas2_her_gtest, her_strided_batched_gtest_double)
{
    // GetParam return a tuple. Tee setup routine unpack the tuple
    // and initializes arg(Arguments) which will be passed to testing routine
    // The Arguments data struture have physical meaning associated.
...
INSTANTIATE_TEST_SUITE_P(hipblasHer,
                         blas2_her_gtest,
