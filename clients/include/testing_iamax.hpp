...
template <typename T>
hipblasStatus_t testing_amax(const Arguments& argus)
{
    ...
    if(N < 1 || incx <= 0)
    {
        ...
        status_1 = hipblasIamaxFn(handle, N, dx, incx, &rocblas_result1);
    ...
    else
    {
        ...
        // device_pointer for d_rocblas_result
        {
            ...
            status_1 = hipblasIamaxFn(handle, N, dx, incx, d_rocblas_result);
        ...
        if((status_1 == HIPBLAS_STATUS_SUCCESS) && (status_3 == HIPBLAS_STATUS_SUCCESS))
        {
            ...
            status_2 = hipblasIamaxFn(handle, N, dx, incx, &rocblas_result2);
        }
