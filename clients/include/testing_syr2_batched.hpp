...
template <typename T>
hipblasStatus_t testing_syr2_batched(Arguments argus)
{
    bool FORTRAN = argus.fortran;
    auto hipblasSyr2BatchedFn = FORTRAN ? hipblasSyr2Batched<T, true> : hipblasSyr2Batched<T, false>;
    ...
    if(argus.unit_check)
    {
        ...
        for(int b = 0; b < batch_count; b++)
        {
            cblas_syr2<T>(uplo, N, alpha, hx[b], incx, hy[b], incy, hA_cpu[b], lda);
        }
        ...
        if(argus.unit_check)
        {
            unit_check_general<T>(N, N, batch_count, lda, hA, hA_cpu);
        }
    }
