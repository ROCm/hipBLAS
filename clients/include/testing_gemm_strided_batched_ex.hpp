...
hipblasStatus_t testing_gemm_strided_batched_ex_template(hipblasOperation_t transA,
                                         ...
                                         bool               FORTRAN)
{
    auto hipblasGemmStridedBatchedExFn = FORTRAN ? hipblasGemmStridedBatchedExFortran : hipblasGemmStridedBatchedExFortran;
    ...
    if(unit_check)
    {
        for(int b = 0; b < batch_count; b++)
        {
            unit_check_general<Td>(M, N, ldc, hC_gold.data() + b * stride_C, hC.data() + b * stride_C);
        }
    }
