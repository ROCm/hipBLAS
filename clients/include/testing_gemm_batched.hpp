...
hipblasStatus_t testing_GemmBatched(Arguments argus)
{
    ...
    {   
        ...
        if ((err_A != hipSuccess) || (err_B != hipSuccess) || (err_C_1 != hipSuccess) || (err_C_2 != hipSuccess))
        {
            CLEANUP();
            hipblasDestroy(handle);
        ...
        err_C_1 = hipMemcpy(dC1_array[i], hC_array[i], sizeof(T)*C_mat_size,  hipMemcpyHostToDevice);
        err_C_2 = hipMemcpy(dC2_array[i], hC_array[i], sizeof(T)*C_mat_size,  hipMemcpyHostToDevice);
    ...
    err_C_1 = hipMemcpy(dC1_array_dev, dC1_array, batch_count * sizeof(*dC1_array), hipMemcpyHostToDevice);
    err_C_2 = hipMemcpy(dC2_array_dev, dC2_array, batch_count * sizeof(*dC2_array), hipMemcpyHostToDevice);
    if ((err_A != hipSuccess) || (err_B != hipSuccess) || (err_C_1 != hipSuccess) || (err_C_2 != hipSuccess))
    {
        CLEANUP();
        hipblasDestroy(handle);
    ...
    // test hipBLAS batched gemm with alpha and beta pointers on host
    {
        ...
        {   
            ...
            err_C_2 = hipMemcpy(hC_array[i], dC2_array[i], sizeof(T)*C_mat_size,  hipMemcpyDeviceToHost);
    ...
    // test hipBLAS batched gemm with alpha and beta pointers on device
    {
        ...
        {   
            // copy result matrices from device to host
            err_C_1 = hipMemcpy(hC_array[i], dC1_array[i], sizeof(T)*C_mat_size,  hipMemcpyDeviceToHost);
