...
hipblasStatus_t testing_getri_strided_batched(Arguments argus)
{
    bool FORTRAN = argus.fortran;
    auto hipblasGetriStridedBatchedFn
        = FORTRAN ? hipblasGetriStridedBatched<T, true> : hipblasGetriStridedBatched<T, false>;
    ...
    for(int b = 0; b < batch_count; b++)
    {
        ...
        hInfo[b] = cblas_getrf(M, N, hAb, lda, hIpivb);
    }
    ...
    status = hipblasGetriStridedBatchedFn(
    ...
    if(argus.unit_check)
    {
        ...
        for(int b = 0; b < batch_count; b++)
        {
            ...
            cblas_getri(
                N, hA.data() + b * strideA, lda, hIpiv.data() + b * strideP, work.data(), -1);
            int lwork = type2int(work[0]);

            // Perform inversion
            work     = host_vector<T>(lwork);
            hInfo[b] = cblas_getri(
                N, hA.data() + b * strideA, lda, hIpiv.data() + b * strideP, work.data(), lwork);
