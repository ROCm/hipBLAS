...
hipblasStatus_t testing_asum(const Arguments& argus)
{
    ...
    status_2 = hipblasAsum<T, Tr>(handle, N, dx, incx, d_hipblas_result);

    status_1 = hipblasSetPointerMode(handle, HIPBLAS_POINTER_MODE_HOST);
    status_2 = hipblasAsum<T, Tr>(handle, N, dx, incx, &hipblas_result_host);
    ...
    if(argus.timing)
    {
        ...
        for(int iter = 0; iter < runs; iter++)
        {
            ...
            status_1 = hipblasAsum<T, Tr>(handle, N, dx, incx, d_hipblas_result);
