...
getopt -T
if [[ $? -eq 4 ]]; then
  GETOPT_PARSE=$(getopt --name "${0}" --longoptions help,install,clients,dependencies,debug,hip-clang,cuda,cmakepp,relocatable:,rocm-dev:,rocblas: --options rhicdgp:v:b: -- "$@")
else
